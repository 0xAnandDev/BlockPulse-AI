...
import { askAssistant } from '../../lib/api/walletDetails'
...
const SUGGESTIONS = [
  'Why is my wallet considered high risk?',
  'What should I do next?',
  'Tell me about recent activity.',
]

export default function WalletAssistantChat({ walletId, walletName }: { walletId: string; walletName: string }) {
  const [messages, setMessages] = useState<Array<ChatMessage>>([
    {
      id: 'welcome',
      role: 'assistant',
      text: `Hi, I'm your AI security assistant for ${walletName}. Ask me why it's rated the way it is, what to do next, or about approvals, ownership changes, or recent activity.`,
    },
  ])
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      const reply = await askAssistant(walletId, text.trim())
      setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: 'assistant', text: reply.answer }])
    } catch (err) {
  ...
  return (
    <div className="panel flex h-[55vh] flex-col rounded-2xl">
      <div className="border-b border-[var(--line)] px-5 py-4">
        <p className="kicker mb-1">AI security assistant</p>
        <h2 className="display-title text-lg font-bold text-[var(--ink)]">Ask about this wallet</h2>
      </div>
      ...
      >
        <input
          ...
          placeholder="Ask about this wallet's risk, alerts, or approvals..."
